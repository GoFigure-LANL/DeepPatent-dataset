...
def eval_model(db, net, trfs, pooling='mean', gemp=3, detailed=False, whiten=None,
    ...
    # Some models (e.g. ImageNet trained ResNet50) return a row of NaN values for an image or two, causing an error in evaluation.
    # if  np.isnan( np.sum(qdescs) ):
    #     qdescs[np.where( np.isnan( qdescs ) )] = 0
    # if  np.isnan( np.sum(bdescs) ):
    #     bdescs[np.where( np.isnan( bdescs ) )] = 0            
